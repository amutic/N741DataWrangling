...
* vector
* matrices (and "arrays")
* data frame- the workhorse for r
* list
* factors
...
**r is case-sensitive**
**if new variable is in " " it will show up as a character variable**
...
typeof(v.int)
```
** double means how it was stored in computer**
---
...
```{r}
is.integer(v.int)
# if you want to check if there is 1/2 a person for ex
v.int2 <- as.integer(v.int)
# this will force it to be an integer
is.integer(v.int2)
```
...
# if you want to make it a character for ex
...
mode(df)
typeof(df)
attributes(df) #kinda like proc contents
```
...
# how to check your session info (which is imp if you need to post a question on the discussion board)
```{r}
sessionInfo()
```
...
#suppose I wanted to read in a SPSS file
#so I want to read_spss() function inside
#the haven package. Load have by using library command
library(haven)
...
```
# this is helping you create a "codebook"
...
#force (or make) it to be a number then calculate the mean
